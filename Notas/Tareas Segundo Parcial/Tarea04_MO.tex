...
\begin{document}
	...
	\begin{enumerate}
		...
		\textbf{Solución:}
		Supongamos dos ondas planas monocromáticas con la misma longitud de onda $\lambda$ y vectores de onda $\vec{k}_1$ y $\vec{k}_2$ que se cruzan formando un ángulo $\varphi$. Haremos que el plano de propagación sea $xz$ y que la bisectriz del ángulo coincida con el eje $z$.
		
		Los vectores de onda se definen como:
		$$ \vec{k}_1 = \frac{2\pi}{\lambda} \left( \sin\left(\frac{\varphi}{2}\right)\hat{i} + \cos\left(\frac{\varphi}{2}\right)\hat{k} \right) $$
		$$ \vec{k}_2 = \frac{2\pi}{\lambda} \left( -\sin\left(\frac{\varphi}{2}\right)\hat{i} + \cos\left(\frac{\varphi}{2}\right)\hat{k} \right) $$
		
		La diferencia de fase óptica $\Delta\phi$ entre las dos ondas en un punto $\vec{r} = x\hat{i} + y\hat{j} + z\hat{k}$ está dada por:
		$$ \Delta\phi = (\vec{k}_1 - \vec{k}_2) \cdot \vec{r} $$
		$$ \vec{k}_1 - \vec{k}_2 = \frac{4\pi}{\lambda} \sin\left(\frac{\varphi}{2}\right)\hat{i} $$
		$$ \Delta\phi = \frac{4\pi}{\lambda} x \sin\left(\frac{\varphi}{2}\right) $$
		
		La separación entre franjas adyacentes ($s$) se define como la distancia $\Delta x$ requerida para que la fase cambie en $2\pi$. Por lo tanto:
		$$ 2\pi = \frac{4\pi}{\lambda} s \sin\left(\frac{\varphi}{2}\right) $$
		Despejando $s$:
		$$ s = \frac{2\pi \lambda}{4\pi \sin(\varphi/2)} = \frac{\lambda}{2\sin(\varphi/2)} $$
		...
		\begin{center}
			\begin{tikzpicture}[font=\small]
				...
				\draw[thick, red, ->, >=stealth] (-2, 1.5) -- (0, 1.5) node[midway, above] {Laser};
				\foreach \a in {-35,-15,0,15,35}
				\draw[red, ->, >=stealth, opacity=0.4] (0.5, 1.5) -- +( \a:2.5 );
				...
				\begin{scope}[xshift=6cm]
					...
					\draw[fill=gray!40] (0,0) rectangle (0.5,3);
					\draw[thick, red, ->, >=stealth] (-2, 1.5) -- (0, 1.5);
					% Lente
				...
				\begin{scope}[yshift=-5cm, xshift=3cm]
					...
					\draw[thick, red, ->, >=stealth] (-2, 1.5) -- (-0.5, 1.5) node[midway, above] {Láser};
					% Ondas cambiantes
					\draw[red, thick, ->, >=stealth] (0.5, 1.5) .. controls (1.5, 2.5) and (2.5, 0.5) .. (3.5, 1.5) node[right, black] {Fluctuaciones temporales};
					\draw[red, thick, dashed, ->, >=stealth] (0.5, 1.5) .. controls (1.5, 0.5) and (2.5, 2.5) .. (3.5, 1.5);
				\end{scope}
			\end{tikzpicture}
		\end{center}
		\vspace{0.5cm}
		...
		\textbf{Solución:}
		\textbf{Diferencia Fundamental:} Un interferómetro \textbf{fuera del plano (Out-of-plane)} está diseñado con vectores de iluminación y observación casi perpendiculares a la superficie de prueba, por lo que su máxima sensibilidad se encuentra en los desplazamientos normales (en el eje $z$). En cambio, un interferómetro \textbf{en el plano (In-plane)} utiliza dos haces simétricos de iluminación incidente para que el vector de sensibilidad sea paralelo a la superficie, volviéndolo sensible únicamente a desplazamientos laterales (ejes $x$ o $y$).
		
		\begin{center}
			\begin{tikzpicture}[font=\small, scale=0.85]
				% OUT OF PLANE
				\node at (2, 5.5) {\textbf{Interferómetro Out-of-plane}};
				% Laser
				\draw[thick, fill=red!20] (-2, 2.5) rectangle (0, 3.5) node[midway] {Láser};
				\draw[thick, red, ->, >=stealth] (0, 3) -- (2, 3);
				% Beam splitter
				\draw[thick, blue, rotate around={-45:(2,3)}] (1.5, 3) rectangle (2.5, 3.1);
				\node at (1.5, 2.2) {BS};
				% Espejo Referencia
				\draw[ultra thick, black] (1.5, 5) -- (2.5, 5) node[above] {Espejo Ref.};
				\draw[thick, red, ->, >=stealth] (2, 3) -- (2, 4.9);
				\draw[thick, red, ->, >=stealth, dashed] (2, 4.9) -- (2, 3);
				% Objeto
				\draw[thick, fill=gray!30] (4.5, 2.5) rectangle (5.5, 3.5);
				\node at (5, 2) {Objeto ($z$)};
				\draw[thick, red, ->, >=stealth] (2, 3) -- (4.4, 3);
				\draw[thick, red, ->, >=stealth, dashed] (4.4, 3) -- (2, 3);
				% Sensor
				\draw[thick, black] (1.5, 0) rectangle (2.5, 0.5) node[below] {CCD};
				\draw[thick, red, ->, >=stealth] (2, 3) -- (2, 0.5);
				
				% IN PLANE
				\begin{scope}[xshift=8cm]
					\node at (3, 5.5) {\textbf{Interferómetro In-plane}};
					% Laser
					\draw[thick, fill=red!20] (-1.5, 5.5) rectangle (0.5, 4.5) node[midway] {Láser};
					\draw[thick, red, ->, >=stealth] (0.5, 5) -- (2.8, 5);
					% Beam splitter
					...
					% Trayectorias
					\draw[thick, red, ->, >=stealth] (3, 5) -- (3, 2.1);
					\draw[thick, red, ->, >=stealth] (3, 5) -- (5.9, 5);
					\draw[thick, red, ->, >=stealth] (3, 2) -- (4.4, 0.5) node[midway, left] {$+\theta$};
					\draw[thick, red, ->, >=stealth] (6, 5) -- (4.6, 0.5) node[midway, right] {$-\theta$};
					% Observacion
					\draw[thick, black] (4, 3.5) rectangle (5, 4) node[above] {CCD normal};
					\draw[thick, red, dashed, ->, >=stealth] (4.5, 0.5) -- (4.5, 3.5);
				\end{scope}
			\end{tikzpicture}
		\end{center}
